...
TEST(FamilyTest, throw_on_invalid_metric_name) {
  auto create_family_with_invalid_name = []() {
    return std::unique_ptr<Family<Counter>>(
        new Family<Counter>("", "empty name", Labels{}));
  };
...
TEST(FamilyTest, throw_on_invalid_constant_label_name) {
  auto create_family_with_invalid_labels = []() {
    return std::unique_ptr<Family<Counter>>(
        new Family<Counter>("total_requests", "Counts all requests",
...
TEST(FamilyTest, reject_histogram_with_constant_le_label) {
  ...
  EXPECT_ANY_THROW(std::unique_ptr<Family<Histogram>>(
      new Family<Histogram>("name", "help", labels)));
}
...
TEST(FamilyTest, reject_summary_with_constant_quantile_label) {
  ...
  EXPECT_ANY_THROW(std::unique_ptr<Family<Summary>>(
      new Family<Summary>("name", "help", labels)));
}
